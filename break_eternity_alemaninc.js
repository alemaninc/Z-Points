...
const format = {     // functions used in formatting
  engineering(x) {
    ...
    return x.div(N(10).pow(exponent)).toPrecision(4)+"e"+exponent.toNumber().toLocaleString("en-US")
  },
  mixedscientific(x) {
    x=N(x)
		x.mag+=Math.min(1e-8,x.mag/1e8) // rogue decimal prevention
    if (x.lt(1e33)) { 
      ...
      return x.div(N(1000).pow(thousand)).toNumber().toPrecision(4)+" "+["","K","M","B","T","Qa","Qt","Sx","Sp","Oc","No"][thousand]
    }
  ...
  scientific(x) {
    x=N(x)
		x.mag+=Math.min(1e-8,x.mag/1e8) // rogue decimal prevention
    return x.log10().mod(1).pow10().mul(1000).floor().div(1000).toPrecision(4)+"e"+x.log10().floor().toNumber().toLocaleString("en-US")
  }
}
